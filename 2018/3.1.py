    ...
    for line in f:
        ...
        for x in range(design.left, design.left + design.width):
            for y in range(design.top, design.top + design.height):
                ...
                if fabric[x][y]!= 0 and design.id not in colisions:
                    colisions.add(design.id)
                if fabric[x][y]!= 0 and fabric[x][y] not in colisions:
                    colisions.add(fabric[x][y])

                fabric[x][y] = design.id
                #printfabric(fabric)
        ids = design.id
f.closed

print(set(range(1, ids + 1)).difference(colisions))
